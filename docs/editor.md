...
**2. Το μέγεθος της πίστας δεν είναι ελεύθερο (§5.2, §9.2, §13).** Η
`MAP_INSTALL` **αρνείται** οτιδήποτε εκτός από **128×16 tiles**, επειδή το
`MAP_CELL` βγάζει τη σειρά από τη base address σε compile time. Τα
προκαθορισμένα μεγέθη «20×N οθόνες × 11» και το παράδειγμα 240×11
περιγράφουν κάτι που η μηχανή δεν φορτώνει. Το `tools/test_format.py` το
κρατά ως negative control: γράψε 64 στο πλάτος και ο loader αφήνει το
`MAP_ADDR` μηδενικό.
...
- ~~Προκαθορισμένα μεγέθη: οριζόντια 20×N οθόνες × 11, κάθετη 20 × 11×N, ή ελεύθερο μέγεθος.~~
  **Η μηχανή φορτώνει 128×16 tiles και τίποτε άλλο — §0.2.**
...
- **Χάρτης**: 1 byte/tile, row-major (**128×16 = 2.048 bytes — §0.2**, ασυμπίεστος:
...
- Μέγιστο αναμενόμενο επίπεδο: **128×16 = 2.048 tiles** (§0.2· ~~12 οθόνες οριζόντια 240×11~~),
